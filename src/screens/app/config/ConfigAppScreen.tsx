...
export default function ConfigAppScreen({ navigation }: any) {
    ...
    const OptionItem = ({ icon, label, onPress, value, isSwitch, color }: any) => (
        <TouchableOpacity
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.text, marginTop: 24 }]}>Minha Conta</Text>
                ...
                <OptionItem
                    ...
                    label="Sobre a Includ.IA"
                    onPress={() => navigation.navigate('AboutApp')}
                />

                <TouchableOpacity style={[styles.logoutBtn, { borderColor: '#FF3B30', backgroundColor: 'rgba(255, 59, 48, 0.05)' }]} onPress={handleLogout}>
                    <Ionicons name="log-out-outline" size={20} color="#FF3B30" />
                    <Text style={styles.logoutText}>Sair da Conta</Text>
                </TouchableOpacity>

                <Text style={styles.version}>Versão 1.0.0 (Build 2025)</Text>
            </ScrollView>
        </SafeAreaView>
    );
}
